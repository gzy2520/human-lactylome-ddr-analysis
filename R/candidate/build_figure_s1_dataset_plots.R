...
p_s1b_main <- ggplot() +
  ...
  geom_text(
    ...
    inherit.aes = FALSE, family = publication_font, size = 3.3, fontface = "italic", colour = "#98A1AA"
  ) +
  scale_fill_manual(values = category_colours, guide = "none") +
...
# =========================================================================
message(">>> Generating companion Figure S1b (only 11 detected datasets)...")
...
fig_s1b_det <- (p_s1b_det_track / p_s1b_det_main) +
  plot_layout(heights = c(0.9, 12)) +
  plot_annotation(
    title = "MKI67 / H3C1 protein intensity ratio across 11 quantified whole-proteome datasets",
    theme = theme(
      plot.title = element_text(family = publication_font, size = 16, face = "bold", colour = charcoal, hjust = 0.5, margin = margin(t = 6, b = 4))
    )
  )
