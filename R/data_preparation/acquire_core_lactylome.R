#!/usr/bin/env Rscript
# =============================================================================
# acquire_core_lactylome.R — source acquisition and reference-pairing utilities
#
...
#
# 用法：
#   Rscript R/data_preparation/acquire_core_lactylome.R <project_root> [--stage <name>]
...
# Shared helpers (accession_utils -> io_utils -> extractors).
lib_dir <- file.path(project_root, "R", "utils")
...
run_build_ensembl_uniprot_mapping <- function() {
  output_path <- file.path(
    project_root,
    "config", "ensembl_protein_to_uniprot_biomart.tsv"
  )
  unmapped_path <- file.path(
    project_root,
    "config", "ensembl_protein_unmapped_biomart.tsv"
  )
...
run_build_human_lactylome_inventory <- function() {
  config_path <- file.path(project_root, "config", "lactylome_manual_curation.csv")
...
run_build_healthy_special_reference_manifest <- function() {
  config <- read.csv(
    file.path(
      project_root,
      "config/healthy_special_reference_catalog.csv"
    ),
    stringsAsFactors = FALSE,
    check.names = FALSE
  )
  output_path <- file.path(
    project_root,
    "results/tables/healthy_special_reference_acquisition_manifest.csv"
  )
...
run_build_lactylome_reference_pairing <- function() {
  ...
  report <- c(
    ...
    "## 输出",
    "",
    "- results/tables/lactylome_and_reference_proteome_pairing_zh.csv",
    "- results/tables/lactylome_and_reference_proteome_gaps_zh.csv",
    "- results/tables/lactylome_reference_all_gaps_zh.csv",
    "- results/tables/lactylome_group_two_reference_columns_complete_zh.csv",
    "- results/tables/lactylome_dataset_decisions_zh.csv",
    "- results/tables/lactylome_reference_pairing_summary_zh.csv",
    "- results/tables/lactylome_and_reference_proteome_pairing_zh.xlsx"
  )
...
run_summarize_acquired_lactylome_data <- function() {
  ...
  report <- c(
    ...
    "## 主要交付",
    "",
    "- 人源乳酸化质谱总表：results/tables/human_lactylome_mass_spectrometry_inventory.xlsx",
    "- 92 个 PXD 的机器可读总表：results/tables/human_lactylome_mass_spectrometry_inventory.csv",
    "- 仓库文件与来源 URL：results/tables/human_lactylome_repository_file_manifest.csv",
    "- 当前已获取数据 QC：results/tables/priority_dataset_acquisition_qc_summary_zh.csv",
    "- 当前下载文件登记：results/tables/priority_dataset_acquisition_manifest.csv",
    ""
  )
  writeLines(
    report,
...
run_register_additional_lactylome_pair_files <- function() {
  config_path <- file.path(
    project_root,
    "config/additional_lactylome_pair_files.csv"
  )
  table_dir <- file.path(project_root, "results/tables")
