...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath(
    "R/figures/plot_bp_semantic_umap.R",
    mustWork = TRUE
  )
}
...
go_long_path <- file.path(v3_table_dir, "direct_go_annotations.csv")
plot_data_path <- file.path(
  v3_table_dir,
  "protein_metadata_and_scores.csv"
)
assignment_path <- file.path(v3_table_dir, "pathway_assignments.csv")
color_path <- file.path(v3_table_dir, "pathway_colors.csv")
...
report_path <- file.path(
  project_root,
  paste0(
    "results/reports/UMAP_PATHWAY_PIE_33GROUP_V4_BP_SEMANTIC",
...
plot_data <- merge(
  ...
  plot_source[
    ,
    setdiff(names(plot_source), c("UMAP_1", "UMAP_2")),
    with = FALSE
  ],
  by = "BaseAccession",
  all.x = TRUE,
...
fwrite(
  bp_binary_dense,
  file.path(table_dir, "bp_semantic_binary_matrix.csv")
)
fwrite(
  raw_coordinates,
  file.path(table_dir, "umap_raw_coordinates.csv")
)
fwrite(
  display_coordinates,
...
fwrite(
  pathway_info,
  file.path(table_dir, "pathway_colors.csv")
)
...
report_lines <- c(
  ...
  "## 保存的GO表和矩阵",
  "",
  "- UniProt原始全GO返回表：`config/uniprot_kla_ddr_507_all_go_2026-08-10.tsv`",
  "- UniProt下载元数据：`config/uniprot_kla_ddr_507_all_go_2026-08-10_metadata.tsv`",
  "- 全部直接蛋白–GO长表：`results/tables/protein_function_inputs/direct_go_annotations.csv`",
  "- 507 × 3,461全部直接GO矩阵：`results/tables/protein_function_inputs/direct_go_binary_matrix.csv`",
  "- BP直接term到祖先节点展开表：`results/tables/bp_semantic_umap/bp_direct_to_ancestor_expansion_long.csv`",
  "- V4实际使用的BP语义矩阵：`results/tables/bp_semantic_umap/bp_semantic_binary_matrix.csv`",
  "",
  "## 图形输出",
  "",
  "- 推荐饼图：`results/figures/bp_semantic_umap/kla_ddr_pathway_pie_umap_v4_bp_semantic.{png,pdf,svg}`",
  "- 未位移原始点图：`results/figures/bp_semantic_umap/kla_ddr_raw_bp_semantic_umap_v4.{png,pdf,svg}`",
  "- 原始与展示对照：`results/figures/bp_semantic_umap/kla_ddr_raw_and_pie_bp_semantic_umap_v4.{png,pdf,svg}`"
)
