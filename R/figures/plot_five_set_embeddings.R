...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath(
    "R/figures/plot_five_set_embeddings.R",
    mustWork = TRUE
  )
}
...
venn_table_dir <- file.path(
  project_root,
  "results/tables/four_class_venn/kla_ddr_four_class_venn"
)

feature_matrix_path <- file.path(
  v4_source_table_dir,
  "bp_semantic_binary_matrix.csv"
)
...
assignment_path <- file.path(
  v3_source_table_dir,
  "pathway_assignments.csv"
)
metadata_path <- file.path(
  v3_source_table_dir,
  "protein_metadata_and_scores.csv"
)
color_path <- file.path(v4_source_table_dir, "pathway_colors.csv")

analysis_name <- "five_set_embeddings"
...
if (length(missing_inputs) > 0L) {
  stop(
    ...
    "\nRun R/analysis/tune_five_set_embeddings.R first."
  )
}
