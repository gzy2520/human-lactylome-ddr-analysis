...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath(
    "R/figures/plot_pathway_specific_umap.R",
    mustWork = TRUE
  )
}
...
metadata_path <- file.path(v3_table_dir, "protein_metadata_and_scores.csv")
coordinate_path <- file.path(v4_table_dir, "umap_raw_coordinates.csv")
color_path <- file.path(v4_table_dir, "pathway_colors.csv")

analysis_name <- "pathway_specific_umap"
