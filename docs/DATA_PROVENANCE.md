...
- 数据集总表：`results/tables/dataset_analysis_summary.csv`
- 每个 PXD 的 sample-level Kla：`work/intermediate/kla_by_dataset/`
- 每个 PXD 的去重 Kla 蛋白：`results/tables/per_pxd/`
- 每个 PXD 的 Kla 与 GO 交集：`work/intermediate/go_intersection/`
- 全部 Kla 蛋白：`results/tables/all_unique_kla_proteins.csv`
- GO 未匹配蛋白：`results/tables/go_unmatched_kla_proteins.csv`
- 精确 Venn 区域：`results/tables/venn_regions/all_kla/` 和 `results/tables/venn_regions/kla_go_ddr/`
- 肿瘤特异 Kla：`results/tables/tumor_specific_kla_proteins.csv`
- 肿瘤特异 Kla-GO：`results/tables/tumor_specific_kla_ddr_proteins.csv`
- PXD028488 目录审计：`results/tables/pxd028488/`
- PXD053474 检索与 S3 对照：`results/tables/pxd053474/`
- MRE11/XLF/NBS1 源表追踪：`results/tables/target_protein_source_level_audit_MRE11_XLF_NBS1.csv`
- 排除日志：`results/tables/exclusion_log.csv`
- 自动测试日志：`work/logs/test_run_2026-07-22.log`
...
统一入口为 `workflow/run_pipeline.R`。底层7个核心PXD的异构Kla表由
`python/data_preparation/build_core_kla_inputs.py` 解析，具体规则位于
`python/utils/extractors.py`；33组主分析及全部发表图由 `R/` 下脚本完成。
