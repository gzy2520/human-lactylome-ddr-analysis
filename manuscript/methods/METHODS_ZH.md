...
四集合 Venn 分析。最终 30 组依次分为肿瘤组织 2 组、非肿瘤组织 9 组、
癌细胞系 12 组和正常细胞系 7 组。
...
肿瘤组织、癌细胞系、正常细胞系以及 399 蛋白全集，共 4+1 张独立线性图。评分表中
人工界定的 7 条 DDR 通路（BER、NER、MMR、FA、HR、AEJ 和 NHEJ）保持不变。
状态 +1、-1 和 0 分别用通路实色、深炭灰和浅灰表示。加权总分只用于每张图中
蛋白的升序排列，不参与通路状态或颜色计算。
...
和 region 表精确复算。本轮不重新生成 UMAP、t-SNE、PCA 和 Cytoscape 图。
最终文件使用 SHA256 manifest 记录。
...
`results/figures/`，表格位于 `results/tables/`。
