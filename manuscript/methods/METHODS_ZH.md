...
每个可用的组织或细胞样本组配对材料身份相符的非 Kla 富集普通全蛋白质谱数据。
参照关系包括同一批实际样本、同研究匹配样本、材料匹配基线和独立队列，因此实验
状态匹配情况另行记录。方法部分的组织方式参考 Hu 等发表于 *Genome Biology* 的生信方法学
文章，即依次说明数据来源与预处理、分析判定规则、定量指标、可视化、计算环境
和可复现性；具体算法和文字均根据本项目重新撰写。

分析以 `PXD+SampleGroup` 为最小研究单元。来源审计中共有 37 个样本组具有可
追溯的 Kla 定量，其中 30 个进入最终 Kla/普通全蛋白热图、DDR 占比比较和
四集合 Venn 分析。按显示顺序，最终30组依次为非肿瘤组织9组、肿瘤组织2组、
癌细胞系12组和正常细胞系7组。
...
有效 Kla 证据并执行上述基础质量控制。PXD014870 因实验被判断为基本失败且仅
捕获 193 个 Kla 蛋白，显著少于另一组 MCF7 Kla 数据，因此不进入最终主分析。
...
2. 同一研究、同一细胞或组织的数据；
3. 材料匹配的基线数据；
4. 独立研究中材料身份和解剖粒度相同的数据。

“组织相近”或“细胞来源相似”不视为材料匹配。材料身份、供体/队列关系、疾病
状态和处理状态分别审计；除非来源数据支持，不把材料匹配参照统一描述为同实验
状态对照。每个 Kla 样本组的具体材料、处理、参照 PXD、实际读取文件和样本列均
经过材料身份审计。PXD062720、
...
此外，PXD055230 和 PXD057709 属于同一疱疹病毒感染论文家族，病毒感染显著
改变乳酸生物学，并且这两项深度数据主导正常细胞 Kla 并集，因此按老师要求
同时排除；原始文件和处理结果仍保留用于审计。

30 个 Kla 样本组对应 28 个唯一普通全蛋白参照行。HK-2 和 HCT116 的部分 Kla
研究共享完全相同的普通全蛋白数据；样本级配对表保留 30 条关系，普通全蛋白
热图只显示 28 条唯一参照，Kla 热图保留 30 行并按相应参照行连续排列。
...
两套热图均按非肿瘤组织、肿瘤组织、癌细胞系、正常细胞系排列，列按
Writer、Eraser、Writer-Eraser 和 Reader 分面。Kla 热图的行名按 Kla 数据集
自身的材料与实验条件标注；普通全蛋白热图则独立使用参照数据集自身的材料与条件，
并在行名后附相应 PXD。Kla 热图使用白色至蓝色的冷色标度，普通全蛋白热图使用
白色至橙红色标度，颜色越深表示百分位越高。中文和英文版本使用相同数据和轴顺序。

## 四集合 Venn 分析

分别对全部 Kla 蛋白、Kla-DDR 蛋白、普通全蛋白和普通全蛋白-DDR 蛋白构建
四个类别集合。每个集合先按 BaseAccession 去重，再计算所有精确交集区域。
图形使用`ggVennDiagram`根据精确membership绘制固定几何的四集合表示，保留
全部15个逻辑区域，包括计数为0的区域。区域面积不用于表达数量；图中数字来自
精确membership，不受面积拟合压缩影响。每套分析同时输出`membership.csv`、
`region_counts.csv`和`set_counts.csv`，因此所有数字均可由表格独立复算。
`eulerr`仅保留用于输出历史面积拟合审计表，不用于最终展示图形。

## 直接GO term通路线性图

最终 30 组 Kla-DDR 蛋白并集包含 399 个唯一 BaseAccession。分别绘制非肿瘤组织、
...
单个蛋白在某条通路的得分为该通路中不同直接GO term的数量。七通路term数之和
仅用于各面板内的升序排列，同分时以BaseAccession稳定排序。至少命中一个直接
term时使用相应通路实色，否则使用浅灰。`Others`用于完整性审计，不作为第八行
绘制。旧人工±1评分表和通路系数不参与本版本。

## 软件、复现与质量控制

主要分析在 R 4.4.3 中完成，使用 dplyr 1.2.0、readr 2.2.0、tidyr 1.3.2、
ggplot2 4.0.2、ggVennDiagram 1.5.7、eulerr 7.0.4、GO.db 3.20.0、
readxl 1.4.5 和 digest 0.6.39。异质检索结果
的规则提取和基础证据构建使用 Python 3.12.10。所有代码、配置、输入来源、
中间表、命令记录和测试均保存在项目目录。

自动测试验证最终 Kla 轴为 30 行、普通全蛋白轴为 28 行、四分类数量为
9/2/12/7（按图中顺序）、七个排除组未进入最终分析、GeneSymbol 回退数为 0、样本内
百分位具有真实变化、共享参照未被重复显示，以及所有 Venn 数字均可由 membership
和 region 表精确复算。通路合同还验证399个蛋白、10,605个直接蛋白–GO配对、
2,785个唯一term、103个至少进入一条七通路的term、8个多通路term、七通路与
`Others`完整覆盖以及35行五集合summary。本轮不重新生成 UMAP、t-SNE、PCA
和 Cytoscape 图。最终文件使用 SHA256 manifest 记录。

## 数据和代码可得性

公开质谱数据的 PXD、文章 DOI、下载链接和本地文件路径见
`kla_and_reference_teacher_review_zh.csv`。当前有效代码、重跑顺序和关键中间
文件见项目根目录`PROJECT_INDEX.md`和`NEW_CHAT_PROJECT_PROMPT.md`。最终双语图
位于`results/figures/`，表格位于`results/tables/`。归档中的33组、507蛋白、
降维、Cytoscape和人工评分材料仅供历史追溯，不属于当前结果合同。
