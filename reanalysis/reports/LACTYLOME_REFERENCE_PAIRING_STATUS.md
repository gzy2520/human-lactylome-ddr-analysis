...
- 配对表按样本组拆为 56 行。
- 已取得全局 Kla 蛋白明细的 40 个样本组；Kla 是否可分析与普通全蛋白参照是否合格分别记录。
- 其中 33 个样本组同时具有生物材料匹配的逐蛋白强度普通全蛋白参照。
- 严格参照排除表共 7 行；排除普通参照不会删除对应 Kla 证据。
- 6 个纳入样本组仍缺可审计 Kla 蛋白明细，涉及 5 个 PXD：PXD037530、PXD046344、PXD053029、PXD063945、PXD065831。
- 39 个要求纳入的样本组均已配置可计数的健康组织基线。

## 主要缺口

- 当前缺口集中在远程超大处理包、只提供 Spectronaut SNE 的项目，以及仓库没有可用处理结果的项目。
- 0 个可成对样本组使用健康器官代理普通蛋白组；这些记录均标成“健康器官代理”，不是同细胞系或同患者精确匹配。
