...
from common import base_accession, best_annotation, clean_text, read_go_annotations, unique_join


PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_ROOT = PROJECT_ROOT / "data"
REANALYSIS_ROOT = PROJECT_ROOT / "reanalysis"
TABLES = REANALYSIS_ROOT / "results/tables"
FIGURES = REANALYSIS_ROOT / "results/figures"
INTERMEDIATE = REANALYSIS_ROOT / "intermediate/reference_proteomes"
CONFIG_PATH = REANALYSIS_ROOT / "config/reference_proteome_selection.json"
...
def main() -> None:
    ...
    comparison = statistics_frame.merge(kla, on="CellOrTissueType", how="left", validate="one_to_one")
    ...
    if len(comparison) != 10 or comparison["ReferenceDdrFraction"].isna().any():
        raise AssertionError("Reference DDR statistics are incomplete")
    if not (
        comparison["ReferenceDdrProteinCount"]
        <= comparison["ReferenceProteinCount"]
    ).all():
