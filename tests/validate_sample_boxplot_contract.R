...
stop_if(nrow(values) == 98L, "The sample-level candidate must contain 98 observations.")
stop_if(nrow(registry) == nrow(values), "The source registry and value table have different observation counts.")
stop_if(nrow(reconciliation) == 31L, "The sample-level candidate must cover 31 publication groups.")
...
stop_if(nrow(counts) == 31L, "Expected sample-count table does not match the 31-group scope.")
...
stop_if(all(category_check$N_expected == category_check$N_actual),
  "Publication category counts are not 9/3/12/7.")
...
message("PASS: sample-level boxplot inputs cover 98 observations across 31 publication groups.")
