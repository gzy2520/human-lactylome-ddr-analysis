...
paths<-c(script='workflow/full_rna_fraction/train.R',rna=file.path(inp,'full_rna.rds'),split='outputs/20260926_nonlinear_samples_final/tables/split.csv')
bundle<-readRDS(paths['rna']);s<-bundle$metadata;x<-as.data.frame(bundle$x);genes<-colnames(x);y<-s$ObservedPercent;rm(bundle);gc(FALSE)
...
for(mt in unique(c(64L,as.integer(sqrt(length(genes)))))){
...
bas<-copy(p[Model=='RF_mtry64']);bas[,Model:='NoRNA'];bas[Evaluation=='Training fit',Predicted:=wm(seq_len(nrow(s)))];bas[Evaluation=='Within-material test',Predicted:=wm(tr)]
