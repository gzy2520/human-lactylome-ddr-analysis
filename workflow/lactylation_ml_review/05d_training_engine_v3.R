# 05d_training_engine_v3.R
# Reusable, strictly isolated training and evaluation engine for B-R v3.
...
build_design_matrix_v3 <- function(dt, spec) {
  n <- nrow(dt)
...
fit_scalers_v3 <- function(dt_train, mod_col = "mact_generation_lactate") {
  # target_expr
...
apply_scalers_v3 <- function(dt, scalers, mod_col = "mact_generation_lactate") {
  dt_out <- copy(dt)
...
calc_isolated_mp_v3 <- function(dt_train, dt_eval, inner_assign) {
...
train_predict_cv_engine_v3 <- function(d_train,
                                       d_eval,
    ...
    for (m in ridge_models) {
      ...
      for (lam_idx in seq_along(lambda_grid)) {
        ...
        for (f in seq_len(n_folds)) {
          ...
          scalers_A <- fit_scalers_v3(A_train)
          A_scaled <- apply_scalers_v3(A_train, scalers_A)
          V_scaled <- apply_scalers_v3(V_val, scalers_A)
          ...
          mp_inner <- calc_isolated_mp_v3(A_scaled, V_scaled, A_sub_assign)
          A_ready <- mp_inner$train
          V_ready <- mp_inner$eval
          
          # Build design matrices
          XA <- build_design_matrix_v3(A_ready, spec)
          XV <- build_design_matrix_v3(V_ready, spec)
          
          # Fit model on A
          fit_inner <- ridge_glm_v3(XA, A_ready$y, lam_cand)
          if (fit_inner$failed || !fit_inner$converged) {
            fold_losses[f] <- 1e6  # Heavy penalty for failure
          } else {
            preds_V <- pred_ridge_v3(fit_inner, XV)
            fold_losses[f] <- logloss(V_ready$y, preds_V)
          }
        }
        # Equal-weight mean LogLoss across inner folds (matching primary metric)
        loss_by_lam[lam_idx] <- mean(fold_losses)
      }
  ...
  outer_scalers <- fit_scalers_v3(tr)
  tr_scaled <- apply_scalers_v3(tr, outer_scalers)
  ev_scaled <- apply_scalers_v3(ev, outer_scalers)
  
  # MP cross-fitting on ALL d_train across outer inner_assign; ev estimated from ALL d_train
  outer_mp <- calc_isolated_mp_v3(tr_scaled, ev_scaled, inner_assign)
  ...
  for (m in ridge_models) {
    ...
    X_tr <- build_design_matrix_v3(tr_final, spec)
    X_ev <- build_design_matrix_v3(ev_final, spec)
