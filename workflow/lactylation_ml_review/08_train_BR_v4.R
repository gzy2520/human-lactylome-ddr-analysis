# 08_train_BR_v4.R
# Main outer 19-fold cross-validation for B-R v4 using the unified, reusable training engine.
...
# Save copy of contract-verified task and features to v4 output directory
fwrite(d, file.path(out_dir, "BRef_features_v4.csv.gz"))
...
for (cg in conns) {
  ...
  engine_res <- train_predict_cv_engine_v4(
    d_train = tr,
    d_eval = ev,
    models = MODELS,
  ...
  # Actual inner connected-group losses
  il <- copy(engine_res$inner_group_losses)
  il[, OuterConnGroup := cg]
  inner_group_list[[as.character(cg)]] <- il
...
fwrite(rbindlist(inner_group_list), file.path(out_dir, "inner_connected_group_losses.csv"))
...
# Save outputs
fwrite(P, file.path(out_dir, "BRv4_oos_preds.csv.gz"))
fwrite(rbindlist(diag_list), file.path(out_dir, "BRv4_fit_diagnostics.csv"))
fwrite(rbindlist(coef_list), file.path(out_dir, "BRv4_coefficients.csv"))
fwrite(rbindlist(scal_list), file.path(out_dir, "BRv4_scaler_params.csv"))
fwrite(rbindlist(mp_cov_list), file.path(out_dir, "BRv4_MP_coverage.csv"))
if (length(tuning_list) > 0) {
  fwrite(rbindlist(tuning_list), file.path(out_dir, "BRv4_inner_tuning_log.csv"))
}
...
for (m in MODELS) {
  ...
  fwrite(per_fold, file.path(out_dir, paste0("BRv4_perfold_", m, ".csv")))
...
fwrite(perf_summary_dt, file.path(out_dir, "BRv4_performance_summary.csv"))

cat("\n=== B-R v4 Performance Summary ===\n")
...
fwrite(calib_gid_dt, file.path(out_dir, "BRv4_material_calibration_groupid.csv"))
fwrite(calib_ref_dt, file.path(out_dir, "BRv4_material_calibration_referencekey.csv"))
...
fwrite(calib_summary, file.path(out_dir, "BRv4_material_calibration_summary.csv"))
...
cat("\nV4 TRAINING AND EVALUATION COMPLETED SUCCESSFULLY.\n")
