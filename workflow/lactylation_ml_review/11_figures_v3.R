# 11_figures_v3.R
# Publication-quality automated figure generation and source data export for B-R v3.
...
pf_list <- list()
for (m in models_f1) {
  f <- file.path(out_dir, paste0("BRv3_perfold_", m, ".csv"))
  dt <- fread(f)
...
fwrite(pf_all, file.path(out_dir, "fig_v3_perfold_performance_sourcedata.csv"))
...
p1a <- ggplot(pf_all[Model %in% c("M0", "MP", "MP_cal", "ME_P", "ME2P")], 
  ...
  labs(x = "Left-out Connected Component (ConnGroup)", y = "Out-of-fold LogLoss (lower is better)",
...
p1b <- ggplot(pf_all[Model %in% c("M0", "MP", "MP_cal", "ME_P", "ME2P")], 
  ...
  scale_fill_brewer(palette = "Set2") +
  labs(x = "Left-out Connected Component (ConnGroup)", y = "Out-of-fold AP (threshold-aggregated, higher is better)",
...
ggsave(file.path(out_dir, "fig_v3_perfold_performance.png"), p1_combined, width = 11, height = 8.5, dpi = 300)
ggsave(file.path(out_dir, "fig_v3_perfold_performance.pdf"), p1_combined, width = 11, height = 8.5)

# --- Figure 2: Material-Level Calibration across 28 ReferenceKeys ---
cat("Generating Figure 2: Material-Level Calibration...\n")
cal_ref <- fread(file.path(out_dir, "BRv3_material_calibration_referencekey.csv"))
cal_sub <- cal_ref[Model %in% c("M0", "MP", "ME_P", "ME2P")]
cal_sub[, Model := factor(Model, levels = c("M0", "MP", "ME_P", "ME2P"))]

fwrite(cal_sub, file.path(out_dir, "fig_v3_material_calibration_sourcedata.csv"))
...
p2 <- ggplot(cal_sub, aes(x = mean_pred, y = obs_rate, color = Model, shape = Model)) +
  ...
  labs(x = "Mean Predicted Probability P(Kla detected | Ref)",
       ...
       title = "B-R v3 Material-Level Calibration across 28 RNA Reference Materials",
       subtitle = "Each point represents one unique ReferenceKey; dashed diagonal indicates ideal calibration") +
  theme_bw(base_size = 11) +
  theme(panel.grid.minor = element_blank(), legend.position = "none")

ggsave(file.path(out_dir, "fig_v3_material_calibration.png"), p2, width = 7.5, height = 7, dpi = 300)
ggsave(file.path(out_dir, "fig_v3_material_calibration.pdf"), p2, width = 7.5, height = 7)

# --- Figure 3: Module Incremental Effect Attribution ---
cat("Generating Figure 3: Module Incremental Effect...\n")
pf_mep <- fread(file.path(out_dir, "BRv3_perfold_ME_P.csv"))
pf_me2p <- fread(file.path(out_dir, "BRv3_perfold_ME2P.csv"))
pf_me <- fread(file.path(out_dir, "BRv3_perfold_ME.csv"))
pf_me2 <- fread(file.path(out_dir, "BRv3_perfold_ME2.csv"))
...
fwrite(inc_dt, file.path(out_dir, "fig_v3_module_increment_sourcedata.csv"))
...
p3a <- ggplot(inc_dt, aes(x = factor(ConnGroup, levels = 1:19), y = delta_LogLoss, fill = Comparison)) +
  ...
  labs(x = "Connected Component (ConnGroup)",
       ...
       subtitle = "Values > 0 indicate that adding the lactate generation module worsens prediction loss") +
  theme_bw(base_size = 11) +
  theme(legend.position = "top", panel.grid.minor = element_blank())
...
ggsave(file.path(out_dir, "fig_v3_module_incremental_effect.png"), p3_combined, width = 10, height = 8, dpi = 300)
ggsave(file.path(out_dir, "fig_v3_module_incremental_effect.pdf"), p3_combined, width = 10, height = 8)
...
s1_sub <- s1_dt[Model %in% c("MP", "MP_cal", "ME_P", "ME2P"), .(Type = "S1 (Eval Drop)", Model, dropped_group = dropped_test_group, LogLoss = eqw_LogLoss, AP = eqw_AP)]
s2_sub <- s2_sum[Model %in% c("MP", "MP_cal", "ME_P", "ME2P"), .(Type = "S2 (Train Drop)", Model, dropped_group = dropped_train_group, LogLoss = eqw_LL_new, AP = eqw_AP_new)]

sens_all <- rbind(s1_sub, s2_sub)
sens_all[, Model := factor(Model, levels = c("MP", "MP_cal", "ME_P", "ME2P"))]
fwrite(sens_all, file.path(out_dir, "fig_v3_sensitivity_sourcedata.csv"))
...
ggsave(file.path(out_dir, "fig_v3_sensitivity_s1_s2.png"), p4_combined, width = 8.5, height = 7.5, dpi = 300)
ggsave(file.path(out_dir, "fig_v3_sensitivity_s1_s2.pdf"), p4_combined, width = 8.5, height = 7.5)

# Render verification
cat("Verifying rendered figure files...\n")
fig_files <- c(
  "fig_v3_perfold_performance.png", "fig_v3_perfold_performance.pdf",
  "fig_v3_material_calibration.png", "fig_v3_material_calibration.pdf",
  "fig_v3_module_incremental_effect.png", "fig_v3_module_incremental_effect.pdf",
  "fig_v3_sensitivity_s1_s2.png", "fig_v3_sensitivity_s1_s2.pdf"
)
