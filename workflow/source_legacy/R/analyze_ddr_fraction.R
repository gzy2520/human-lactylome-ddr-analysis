...
intermediate_dir <- file.path(
  project_root, "work", "intermediate", "expanded_ddr_by_accession"
)
...
mapping_input <- mapping_input[
  mapping_input$IdentifierType == "ENSEMBLPROT" &
    !is.na(mapping_input$SourceProteinID) &
    nzchar(mapping_input$SourceProteinID),
  ,
  drop = FALSE
]
ensembl_mapping_identifiers <- sort(unique(unlist(lapply(
  mapping_input$SourceProteinID,
  function(value) {
    identifiers <- split_protein_identifiers(value)
    identifiers[grepl("^ENSP[0-9]+$", identifiers)]
  }
))))
mapping_input <- mapping_input[
  !is.na(mapping_input$MappedBaseAccessions) &
    nzchar(mapping_input$MappedBaseAccessions),
  ,
  drop = FALSE
]
ensembl_mapping_records <- bind_rows(lapply(seq_len(nrow(mapping_input)), function(index) {
  identifiers <- split_protein_identifiers(mapping_input$SourceProteinID[[index]])
  identifiers <- identifiers[grepl("^ENSP[0-9]+$", identifiers)]
  accessions <- split_accessions(mapping_input$MappedBaseAccessions[[index]])
  if (!length(identifiers) || !length(accessions)) {
    return(data.frame(
  ...
  expand.grid(
    EnsemblProteinID = identifiers,
    BaseAccession = accessions,
    stringsAsFactors = FALSE
  )
})) |>
  mutate(BaseAccession = base_accession(BaseAccession)) |>
...
kla_scope <- read.csv(
  ...
  transmute(
    乳酸化PXD = PXD,
    样本组 = SampleGroup,
    KlaRowOrder = RowOrder
  )
if (nrow(kla_scope) != 30L || anyDuplicated(kla_scope[c("乳酸化PXD", "样本组")])) {
  stop("The source workflow must use exactly the 30 final publication groups")
}
...
if (nrow(pairing) != 30L) {
  stop("Source pairing must retain all 30 final publication groups")
}
...
message(
  ...
  nrow(statistics),
  " quantifiable Kla groups; ",
  sum(statistics$PairedAnalysisIncluded),
  " paired; ",
  sum(!statistics$PairedAnalysisIncluded),
  " unpaired."
)
